...
if (!empty($_GET['codigoBodega'])) {
    ...
    $sql = "SELECT nombre_bodega FROM bodega where codigo_bodega = ?";
    $q = $pdo->prepare($sql);
...
if (!empty($_POST)) {
    ...
    try {
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "DELETE FROM bodega WHERE codigo_bodega = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($codigoBodega));
        Database::disconnect();
        header("Location: mostrarRegistroBodega.php");
    } catch (Exception $ex) {
                            ...
                            <div class="panel-body">
                                <div class="row">
                                    <div class="col-lg-6">
                                        ...
                                        <form class="form-horizontal" action="../Delete/eliminarBodega.php" method="post">
                                            <input type="hidden" name="codigoBodega" value="<?php echo $codigoBodega; ?>"/>

                                            <?php if (!isset($errorEliminar)) { ?>
                                            <p class="alert alert-error">¿Esta Seguro que desea eliminar la Bodega <b> <?php echo $data['nombre_bodega']; ?></b>?</p>
                                                <div class="form-actions">
                                                    <br>
                                                    <button type="submit" class="btn btn-danger">Si</button>
                                                    <a class="btn btn-default" href="../Show/mostrarRegistroBodega.php">No</a>
                                                </div>
                                            <?php } else { ?>
